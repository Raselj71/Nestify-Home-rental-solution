...
import { OfficeSchema, TOfficeSchema } from "@/utils/zod/OfficeShchema";
...
export default function OfficePropertyForm() {
  ...
  const onSubmit = async (data: TOfficeSchema) => {
     try{
        const url= await uploadImagesAndGetUrls(images)

       const response = await SubmitPropertyAction(data,'office',url)
        console.log(url)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-full">
      ...
      >
        ...
        <LevelSelect
          ...
          required
        />
        <Levelnput type="number" control={control} name="room" error={errors.room} placeholder="Office room" label="Office Room" required size="3"/>
        <Levelnput
          size="3"
          type="number"
          control={control}
